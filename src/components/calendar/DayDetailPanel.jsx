...
export default function DayDetailPanel({ date, onClose, company, experts: allExperts, spaces, workingHours }) {
  ...
  const titleDate = `${parseInt(d)} ${MONTHS[parseInt(m)-1]} ${y}, ${DAYS_FULL[dayOfWeek]}`;
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (selectedUnit?.id && !String(selectedUnit.id).startsWith('session-')) ins.room_unit_id = selectedUnit.id;
  ...
  // Breadcrumb
  const crumbs = [
    { label: 'Gün', active: true },
    selectedService && { label: selectedService.description, active: true },
    selectedRoom && { label: selectedRoom.name, active: true },
    selectedUnit && { label: selectedUnit.name || `Seans ${selectedUnit.sessionTime}`, active: true },
    newAppointment ? { label: `${newAppointment.expert?.name} ${newAppointment.startTime}`, active: true } : null,
  ].filter(Boolean);
  const hint = !selectedService ? 'Hizmet seç' : !selectedRoom ? 'Oda seç' : !selectedUnit ? 'Yatak seç' : !newAppointment ? 'Saat ve personel seç' : null;

  const showGrid = selectedService && (selectedUnit || selectedService.requires_expert === false);
  const svcName = selectedService?.description || '';
    ...
    <div style={{ marginTop: 20, border: '1px solid #e8e8e3', borderRadius: 12, overflow: 'hidden', background: '#fff' }}>
      ...
      <div style={{ padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e8e8e3' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: '#1D9E75', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
            + Randevu oluştur
          </button>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          {showGrid ? (
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 240, color: '#999', fontSize: 14 }}>
              {!selectedService ? 'Soldan bir hizmet seçin' : !selectedRoom ? 'Bir oda seçin' : 'Yatak / alan seçin'}
            </div>
          )}
        </div>
      </div>
      ...
      {newAppointment && selectedService && (
        <div style={{ padding: '12px 20px', borderTop: '1px solid #e8e8e3', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fafaf8' }}>
          <div style={{ fontSize: 13, color: '#1a1a1a' }}>
            <b>{svcName}</b> · {newAppointment.expert?.name} · {newAppointment.startTime} - {newAppointment.endTime} · {selectedService.duration}dk
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <span style={{ fontSize: 11, color: '#999' }}>↕↔ Sürükle taşı</span>
            <button onClick={() => setNewAppointment(null)} style={{
              padding: '8px 16px', borderRadius: 8, border: '1px solid #e8e8e3', background: '#fff',
              color: '#666', fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit',
            }}>İptal</button>
            <button onClick={handleConfirm} disabled={saving} style={{
              padding: '8px 20px', borderRadius: 8, border: 'none', background: '#1D9E75',
...
function MiniBar({ color, pct }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <span style={{ fontSize: 12, fontWeight: 600, fontFamily: "'SF Mono','Menlo',monospace", color: textColor(pct) }}>
        %{pct}
      </span>
    </div>
  );

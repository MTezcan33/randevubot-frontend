...
// Genel hizmet kataloğu — güzellik sektörü
const SERVICE_CATALOG = [
  {
    category: 'Saç Bakımı',
    color: '#E91E8C',
    items: [
      { name: 'Saç Kesimi', duration: 30 },
      { name: 'Fön', duration: 30 },
      { name: 'Saç Boyama', duration: 90 },
      { name: 'Röfle / Balayage', duration: 120 },
      { name: 'Keratin Bakımı', duration: 180 },
      { name: 'Saç Düzleştirme', duration: 90 },
      { name: 'Perma', duration: 120 },
      { name: 'Saç Bakım Maskesi', duration: 45 },
      { name: 'Saç Uzatma', duration: 180 },
    ],
  },
  {
    ...
    color: '#10B981',
    items: [
      { name: 'Yüz Bakımı', duration: 60 },
      { name: 'Derin Temizlik', duration: 60 },
      { name: 'Kimyasal Peeling', duration: 45 },
      { name: 'Nemlendirme Bakımı', duration: 45 },
      { name: 'Anti-Aging Bakımı', duration: 75 },
      { name: 'Akne Bakımı', duration: 60 },
      { name: 'Leke Bakımı', duration: 60 },
      { name: 'Hydrafacial', duration: 90 },
    ],
  },
  {
    ...
    color: '#F97316',
    items: [
      { name: 'Manikür', duration: 45 },
      { name: 'Pedikür', duration: 60 },
      { name: 'Kalıcı Oje', duration: 45 },
      { name: 'Jel Tırnak', duration: 90 },
      { name: 'Protez Tırnak', duration: 90 },
      { name: 'Tırnak Şekillendirme', duration: 30 },
      { name: 'Nail Art', duration: 60 },
    ],
  },
  {
    category: 'Makyaj',
    color: '#D4AF37',
    items: [
      { name: 'Günlük Makyaj', duration: 45 },
      { name: 'Gelin Makyajı', duration: 90 },
      { name: 'Özel Gün Makyajı', duration: 60 },
      { name: 'Kaş Şekillendirme', duration: 20 },
      { name: 'Kaş Boyama', duration: 30 },
      { name: 'Kalıcı Makyaj (Microblading)', duration: 120 },
    ],
  },
  {
    category: 'Masaj',
    color: '#3B82F6',
    items: [
      { name: 'Klasik Masaj', duration: 45 },
      { name: 'İsveç Masajı', duration: 60 },
      { name: 'Derin Doku Masajı', duration: 60 },
      { name: 'Medikal Masaj', duration: 45 },
      { name: 'Aromaterapi Masajı', duration: 45 },
      { name: 'Sıcak Taş Masajı', duration: 90 },
      { name: 'Miks Taş Masajı', duration: 45 },
      { name: 'Bali Masajı', duration: 45 },
      { name: 'Tayland Masajı', duration: 45 },
      { name: 'Lomi-Lomi Masajı', duration: 45 },
      { name: 'Shiatsu Masajı', duration: 45 },
      { name: 'Anti-Stres Masajı', duration: 60 },
      { name: 'Refleksoloji (Ayak Masajı)', duration: 30 },
      { name: 'Baş-Boyun Masajı', duration: 30 },
      { name: 'Hamile Masajı', duration: 60 },
      { name: 'Çift Masajı (Couple)', duration: 60 },
      { name: 'Spor Masajı', duration: 45 },
    ],
  },
  {
    category: 'Hamam & Spa',
    color: '#8B5CF6',
    items: [
      { name: 'Klasik Türk Hamamı', duration: 45 },
      { name: 'VIP Hamam Paketi', duration: 75 },
      { name: 'Kese + Köpük Masajı', duration: 30 },
      { name: 'Köpük Masajı', duration: 20 },
      { name: 'Peeling', duration: 20 },
      { name: 'Sauna', duration: 20 },
      { name: 'Buhar Odası Seansı', duration: 25 },
      { name: 'Tuz Odası Terapi', duration: 40 },
      { name: 'Fin Saunası Seansı', duration: 30 },
      { name: 'Hamam + Masaj Kombo', duration: 90 },
      { name: 'Paket Çınar (VIP)', duration: 90 },
    ],
  },
  {
    category: 'Epilasyon',
    ...
    items: [
      ...
      { name: 'Ağda — Bikini', duration: 20 },
      { name: 'Ağda — Yüz', duration: 20 },
      { name: 'Lazer Epilasyon', duration: 30 },
      { name: 'IPL Epilasyon', duration: 30 },
    ],
  },
  {
    category: 'Kirpik & Kaş',
    color: '#EF4444',
    items: [
      { name: 'Kirpik Lifting', duration: 60 },
      { name: 'Kirpik Boya', duration: 30 },
      { name: 'Kirpik Uzatma (Tek Tek)', duration: 120 },
      { name: 'Kirpik Uzatma (Rus Volüm)', duration: 150 },
      { name: 'Laminasyon', duration: 60 },
      { name: 'Kaş Laminasyonu', duration: 45 },
    ],
  },
];
...
const ServicesPage = () => {
  ...
  // Katalog'dan seçili hizmetleri toplu ekle
  const handleAddFromCatalog = async () => {
    if (selectedCatalogItems.length === 0) return;
    setSaving(true);

    // Her seçili item için hizmet kaydı oluştur
    const inserts = selectedCatalogItems.map(item => ({
      company_id: company.id,
    ...
    const { error } = await supabase.from('company_services').insert(inserts);
    if (error) {
      toast({ title: t('error'), description: t('serviceSaveError', { error: error.message }), variant: 'destructive' });
    } else {
      toast({ title: t('success'), description: `${inserts.length} hizmet eklendi.` });
  ...
  // Katalog item seçim toggle
  const toggleCatalogItem = (item) => {
    setSelectedCatalogItems(prev => {
      const key = `${item.category}__${item.name}`;
      const exists = prev.find(i => `${i.category}__${i.name}` === key);
      if (exists) return prev.filter(i => `${i.category}__${i.name}` !== key);
      return [...prev, item];
    });
  };

  const isCatalogItemSelected = (item) =>
    selectedCatalogItems.some(i => i.category === item.category && i.name === item.name);

  // Filtrelenmiş katalog (arama)
  const filteredCatalog = useMemo(() => {
    if (!catalogSearch) return SERVICE_CATALOG;
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setServiceData({ ...serviceData, service_content: e.target.value })}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setServiceData({ ...serviceData, preparation_info: e.target.value })}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setServiceData({ ...serviceData, contraindications: e.target.value })}
                />
              </div>
      ...
      {/* Hizmet Kataloğu Modalı */}
      <Dialog open={isCatalogOpen} onOpenChange={(open) => {
        setIsCatalogOpen(open);
        if (!open) { setSelectedCatalogItems([]); setCatalogSearch(''); }
      }}>
              ...
              <div key={group.category}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                  {group.items.map(item => {
                    const selected = isCatalogItemSelected(item);
                    return (
                      <button
                        key={item.name}
                        onClick={() => toggleCatalogItem({ ...item, color: group.color })}
